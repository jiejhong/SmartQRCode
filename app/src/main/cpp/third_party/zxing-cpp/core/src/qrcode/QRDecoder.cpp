...
static DecoderResult DecodeWithFormatInfo(const BitMatrix& bits, const Version& version, const FormatInformation& formatInfo, bool requireAllBlocksOk)
{
...
DecoderResult Decode(const BitMatrix& bits)
{
	...
	const auto type = formatInfo.isValid() ? formatInfo.type() : Type::Model2;

	const Version* pversion = ReadVersion(bits, type);
	if (!pversion)
	...
	if (formatInfo.isValid()) {
		auto ret = DecodeWithFormatInfo(bits, version, formatInfo, false);
		if (ret.isValid() && !ret.error())
			return ret;
	}
	...
	for (int mirrored = 0; mirrored <= 1; mirrored++) {
		for (auto ecLevel : {ErrorCorrectionLevel::Low, ErrorCorrectionLevel::Medium, ErrorCorrectionLevel::Quality, ErrorCorrectionLevel::High}) {
			for (int dataMask = 0; dataMask < 8; dataMask++) {
				FormatInformation fi;
				fi.mask = FORMAT_INFO_MASK_MODEL2;
				fi.isMirrored = mirrored != 0;
				fi.dataMask = static_cast<uint8_t>(dataMask);
				fi.ecLevel = ecLevel;
				auto ret = DecodeWithFormatInfo(bits, version, fi, true);
				if (ret.isValid() && !ret.error())
					return ret;
			}
		}
